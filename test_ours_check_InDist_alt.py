...
with open('../state_act_pair_10_imbalanced.pickle', 'rb') as handle:
...
#weights_dir = "./model_weights/clean/pick/ours/pick_and_place_simple-AlarmClock-None-Desk-307"
weights_dir = "../model_weights/clean/pick/imbalanced_ours/pick_and_place_simple-AlarmClock-None-Desk-307"
res_path = weights_dir + '/results_in_dist_alt_2.csv'
...
#             action_ind = np.random.choice(len(actions), 1, replace=False, p=p).item()
        ...
        #TODO: argmax 
            ...
            for a in range(len(actions)):
                ...
                ss.append(a)
                ss = tuple(ss)
                action_counts.append(counter[ss])
            ...
            if action_counts.sum() == 0:
                ...
                action_counts_prob = action_counts/action_counts.sum()

                unormalized = (output[:,1]/output[:,0]) * action_counts_prob
                

                normalized = unormalized/unormalized.sum()
                
                action_ind = np.random.choice(len(actions), p=normalized)
