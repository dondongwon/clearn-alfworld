...
parser.add_argument('-epochs', default = "1001")
...
if state_input == 'Image':
  ...
  #make such that pick doesn't take place at the very start 
...
#get rid of sparsity
  ...
  if use_action == 1:
    ...
    if baseline == 'newLCBC':
      ...
      model = newLCBC(desc_num_classes, action_num_classes)
    
    if baseline == 'twoLCBC':
      model = LCBC2(desc_num_classes, action_num_classes)
...
for epoch in range(epochs):
  ...
  for j,batch in enumerate(tqdm(trainloader)):
    ...
    if use_action >= 1:
      ...
      action = torch.nn.functional.one_hot(action.long(), num_classes= action_num_classes )
    else:
      state, desc, action = batch
    ...
    with torch.no_grad():
      ...
      cond_len = torch.nonzero(label).shape[0]
      acc_cond = torch.mean((predictions[:cond_len] == label[:cond_len]).float())
      acc_marg = torch.mean((predictions[cond_len:] == label[cond_len:]).float())
      loss_cond = loss_fn(output.squeeze()[:cond_len], label[:cond_len].long())
      loss_marg = loss_fn(output.squeeze()[cond_len:], label[cond_len:].long())
    ...
    accur_cond.append(acc_cond)
    accur_marg.append(acc_marg)
    losses_cond.append(loss_cond)
    losses_marg.append(loss_marg)

  epoch_acc_cond = torch.mean(torch.stack(accur_cond))
  epoch_acc_marg = torch.mean(torch.stack(accur_marg))
  epoch_loss_cond = torch.mean(torch.stack(losses_cond))
  epoch_loss_marg = torch.mean(torch.stack(losses_marg))
  ...
  with torch.set_grad_enabled(False):
    ...
    for i,batch in enumerate(tqdm(valloader)):
      ...
      #desc = torch.nn.functional.one_hot(desc, num_classes=len(desc_key))
        action = torch.nn.functional.one_hot(action.long(), action_num_classes)
      else:
        state, desc, action = batch
        state = state.to(device)
        desc = desc.to(device)
      ...
      val_cond_len = torch.nonzero(label).shape[0]
      val_acc_cond = torch.mean((val_predicted[:val_cond_len] == label[:val_cond_len]).float())
      val_acc_marg = torch.mean((val_predicted[val_cond_len:] == label[val_cond_len:]).float())
      val_loss_cond = loss_fn(output.squeeze()[:val_cond_len], label[:val_cond_len].long())
      val_loss_marg = loss_fn(output.squeeze()[val_cond_len:], label[val_cond_len:].long())

      val_losses.append(val_loss.detach())
      val_prop.append(val_prop_ones)
      val_accur.append(val_acc.detach())
      val_accur_cond.append(val_acc_cond)
      val_accur_marg.append(val_acc_marg)
      val_losses_cond.append(val_loss_cond)
      val_losses_marg.append(val_loss_marg)
    ...
    epoch_val_acc_cond = torch.mean(torch.stack(val_accur_cond))
    epoch_val_acc_marg = torch.mean(torch.stack(val_accur_marg))
    epoch_val_loss_cond = torch.mean(torch.stack(val_losses_cond))
    epoch_val_loss_marg = torch.mean(torch.stack(val_losses_marg))
#   print("epoch:", epoch)
